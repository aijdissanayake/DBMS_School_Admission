...
class School extends Model
{
	public function getProximityApplications($regNo){
		...
		// $proxApps = DB::select('select initials, surname, first_name, last_name from applications where active = ?', [1]);
			$proxApps = DB::select('SELECT application_id,initials,denoted_name, surname, total_marks FROM (applications INNER JOIN children ON applications.child_id = children.id) INNER JOIN proximity_applications ON proximity_applications.application_id = applications.id where school_reg_no = ? ORDER BY total_marks DESC', [$regNo]);
	...
	public function getPastPupilApplications($regNo){
			...
			$ppApps = DB::select('SELECT application_id, initials, surname,denoted_name, total_marks FROM (applications INNER JOIN children ON applications.child_id = children.id) INNER JOIN past_pupil_applications ON past_pupil_applications.application_id = applications.id where school_reg_no = ? ORDER BY total_marks DESC', [$regNo]);
